...
class Sprite_Ball(cdkk.Sprite_Shape):
    def __init__(self, posx, posy, velx, vely, *limits):
        super().__init__(name="Ball", rect=cdkk.cdkkRect(posx, posy, 50, 50),
                         style={"fillcolour": "red3", "outlinecolour": None, "shape": "Ellipse"})
...
class Sprite_Obstacle(cdkk.Sprite_Shape):
    def __init__(self, rect):
        super().__init__(name="Obstacle", rect=rect,
                         style={"fillcolour": "green4", "outlinecolour": None, "shape": "Rectangle"})
...
class Manager_Tests(cdkk.SpriteManager):
    ...
    def __init__(self, limits, name="Ball Manager"):
        super().__init__(name)
        self._limits = limits
        self._balls = cdkk.SpriteGroup("Balls")
        self._timer = cdkk.Timer()
        self._next_test = 1
        self._timer = cdkk.Timer()
        tt_style = {"fillcolour": None, "align_horiz": "L",
                    "textformat": "Test: {0} Time Left: {1:0.1f}"}
        self._test_time = cdkk.Sprite_DynamicText(
            "Test+Time", rect=cdkk.cdkkRect(10, 10, 300, 40), style=tt_style)
    ...
    def run_test(self):
        if self._next_test == 1:
            ...
            ball = Sprite_Ball(100, 500, 5, -13,
                               cdkk.Physics_Limit(
                                   self._limits, cdkk.LIMIT_KEEP_INSIDE, cdkk.AT_LIMIT_BOUNCE),
            ...
            ball = Sprite_Ball(100, 500, 6, -20,
                               cdkk.Physics_Limit(
                                   self._limits, cdkk.LIMIT_KEEP_INSIDE, cdkk.AT_LIMIT_BOUNCE),
                               cdkk.Physics_Limit(
                                   obstacle1.rect, cdkk.LIMIT_KEEP_OUTSIDE, cdkk.AT_LIMIT_BOUNCE),
    ...
    def event(self, e):
        ...
        if not dealt_with and e.type == cdkk.EVENT_GAME_CONTROL:
            if e.action == "NextTest":
                self.kill_sprites_by_desc("name", "Ball")
                self.kill_sprites_by_desc("name", "Obstacle")
                self._balls.empty()
                self.run_test()
...
class Sprite_Ball_Old(cdkk.Sprite_Shape):
    def __init__(self, limits, obstacle):
        ...
        self.rect.add_limit(cdkk.Physics_Limit(
            obstacle, cdkk.LIMIT_KEEP_OUTSIDE, cdkk.AT_LIMIT_BOUNCE))
